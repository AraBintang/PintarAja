...
import { Copy, RotateCcw, Upload, ChevronDown, MoreVertical, Check } from 'lucide-react';
        ...
        <div className="flex flex-col h-full bg-[#f7f7f5] dark:bg-gray-900 overflow-y-auto overflow-x-hidden pt-0 pb-10 px-6 max-w-full">
            <div className="max-w-[1200px] mx-auto w-full mb-6 mt-6">
                <h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                    <QuestionIcon className="w-6 h-6 text-emerald-600" />
                    Parafrase AI
                </h1>
                <p className="text-gray-500 text-sm mt-1">Susun ulang teks Anda untuk meningkatkan keterbacaan dan menghindari plagiarisme.</p>
            </div>
            ...
            <div className="max-w-[1200px] mx-auto w-full flex-1 flex flex-col bg-white dark:bg-[#1e2330] rounded-xl border border-gray-200 dark:border-gray-800 overflow-visible min-h-[600px] mb-20 md:mb-8 shadow-sm">
                        ...
                        <div className="flex flex-wrap md:items-center gap-1.5 md:gap-2 w-full lg:w-auto pl-1 pt-1">
                            ...
                            <div className="relative" ref={dropdownRef}>
                                <button
                                    ...
                                    className={`flex items-center gap-1 px-3 py-2 text-[14px] font-medium rounded-[10px] transition-all duration-200 ${moreModes.includes(mode) || moreOpen
                                        ? 'bg-[#eeedeb] dark:bg-gray-800 text-gray-800 dark:text-gray-200'
                                        : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'
                                        }`}
                                >
                                    More {moreModes.includes(mode) ? `(${mode})` : ''} <ChevronDown className="w-4 h-4 ml-1" />
                                </button>
                                {moreOpen && (
                                    <div className="absolute top-full left-0 mt-2 bg-white dark:bg-[#1e2330] border border-gray-100 dark:border-gray-700 shadow-xl rounded-xl flex flex-col min-w-[160px] z-50 overflow-hidden py-1">
                                        {moreModes.map(m => (
                                            <button key={m} onClick={() => { setMode(m); setMoreOpen(false); }} className={`px-5 py-2.5 text-left text-[14px] transition-colors hover:bg-gray-50 dark:hover:bg-[#252b3b] ${mode === m ? 'text-emerald-600 font-semibold bg-emerald-50 dark:bg-emerald-900/20' : 'text-slate-700 dark:text-gray-300'}`}>
                                                {m}
                                            </button>
                                        ))}
                                    </div>
                        ...
                        <Select value={selectedLang} onValueChange={setSelectedLang}>
                            ...
                            <SelectContent className="max-h-[350px] z-50 rounded-xl shadow-xl dark:bg-[#1e2330] dark:border-gray-700">
                                <SelectGroup>
                                    ...
                                    {LANGUAGES.map(lang => (
                                        <SelectItem key={lang.text} value={lang.text} className="cursor-pointer py-2.5 px-3 hover:bg-emerald-50 focus:bg-emerald-50 dark:hover:bg-[#252b3b] dark:focus:bg-[#252b3b] transition-colors rounded-lg mx-1 my-0.5">
                ...
                <div className="flex-1 flex flex-col md:flex-row min-h-0 relative z-10 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <div className="flex-1 flex flex-col border-b md:border-b-0 md:border-r border-gray-100 dark:border-gray-800 bg-white dark:bg-transparent">
                        <div className="flex-1 p-6 relative">
                            ...
                            {!inputText && (
                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-3">
                                    ...
                                    <button
                                        onClick={handlePaste}
                                        className="flex !hidden md:!flex items-center gap-2 px-5 py-2.5 text-[13px] font-semibold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 rounded-full hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition-colors whitespace-nowrap"
                                    >
                        ...
                        <div className="px-6 py-3 border-t border-gray-50 dark:border-gray-800 flex items-center justify-between dark:bg-transparent">
                            <span className={`text-[12px] ${countWords(inputText) >= 125 ? 'text-red-500 font-medium' : 'text-gray-400'}`}>
                            ...
                            {isProcessing ? (
                                <div className="flex items-center justify-center h-full min-h-[250px] md:min-h-0">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <span className="text-[13px] font-medium text-gray-500 dark:text-gray-400">Memparafrase teks...</span>
                                    </div>
                                </div>
                            ) : outputText ? (
                ...
                <div className="hidden md:flex bg-white dark:bg-transparent border-t border-gray-100 dark:border-gray-800 px-6 py-4 flex-col sm:flex-row items-center justify-between gap-4 relative z-10">
                    ...
                    <button
                        ...
                        className={`px-10 py-3 text-[14px] font-bold rounded-full transition-all w-full sm:w-auto ${inputText.trim() && !isProcessing && countWords(inputText) <= 125
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 px-4 pt-3 pb-5 z-[30] transition-all duration-300 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] dark:shadow-[0_-4px_20px_rgba(0,0,0,0.2)]">
                <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between px-2">
                        ...
                        <div className="flex items-center gap-3">
                            <button onClick={handleCopy} className={`p-2 transition-colors ${isCopied ? 'text-emerald-600' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-200'}`}>
                    ...
                    <button
                        onClick={handleParaphrase}
                        disabled={!inputText.trim() || isProcessing || countWords(inputText) > 125}
                        className={`w-full py-3 text-[15px] font-bold rounded-full transition-all ${inputText.trim() && !isProcessing && countWords(inputText) <= 125
