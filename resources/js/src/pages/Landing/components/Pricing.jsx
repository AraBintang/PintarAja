...
import { Sparkles, Video, Crown, Search, BookOpen, Mail, Youtube, FileCode, HardDrive } from 'lucide-react';
            ...
            <div className="max-w-7xl mx-auto px-6 xl:px-0">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-[1200px] mx-auto">
                    {plans.map((plan) => (
                        <div
                            key={plan.id}
                            className={`bg-white dark:bg-gray-800 rounded-[24px] p-8 px-6 lg:px-8 border relative flex flex-col ${plan.id === 1 ? 'border-blue-500 border-2 shadow-lg shadow-blue-100/50' : 'border-gray-200 dark:border-gray-700 shadow-sm'
                                }`}
                        >
                            <div className="text-center mb-6">
                                <h3 className="text-[22px] font-bold text-gray-900 dark:text-white mb-3">{plan.name}</h3>
                                <p className="text-[13px] text-gray-600 dark:text-gray-400 leading-relaxed min-h-[60px]">{plan.description}</p>
                            </div>
                            ...
                            <div className="flex-1">
                                <ul className="space-y-6">
                                    {plan.features.filter(f => f.isIncluded !== false).map((feature, idx) => (
